...
class BaseGlobalCustomFieldPermission(permissions.BasePermission):
    """
    ...
    def check_group_permission(self, request, view):
        ...
        if not Feature.objects.active().filter(
                key=FeatureName.GLOBAL_CUSTOM_FIELD,
                value=True
        ).exists():
            return False
...
class GlobalCustomFieldCreatePermission(BaseGlobalCustomFieldPermission):
    """
...
class GlobalCustomFieldViewPermission(BaseGlobalCustomFieldPermission):
    """
...
class GlobalCustomFieldUpdatePermission(BaseGlobalCustomFieldPermission):
    """
...
class GlobalCustomFieldDestroyPermission(BaseGlobalCustomFieldPermission):
    """
...
class GlobalCustomFieldOwner(permissions.BasePermission):
    """
    ...
    def has_permission(self, request, view):
        ...
        for method in ['POST', 'GET', 'data']:
            if isinstance(getattr(request, method, {}), list):
                for item in getattr(request, method, {}):
                    global_custom_filed_id = item.get('global_custom_field_id', None)
                    if global_custom_filed_id:
                        global_custom_field = get_object_or_404(
                            GlobalCustomField.objects.active(),
                            pk=global_custom_filed_id
                        )
                        return global_custom_field.created_by == request.user
            else:

                global_custom_filed_id = getattr(request, method, {}).get('global_custom_field_id', None)
                if global_custom_filed_id:
                    global_custom_field = get_object_or_404(
                        GlobalCustomField.objects.active(),
                        pk=global_custom_filed_id
    ...
    def has_object_permission(self, request, view, obj):
        ...
        Return `True` if permission is granted, `False` otherwise.
        """
        return obj.global_custom_field.created_by == request.user
